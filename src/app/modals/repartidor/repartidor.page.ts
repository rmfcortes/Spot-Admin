...
export class RepartidorPage implements OnInit {
  ...
  listenResult() {
    this.repartidorService.result(this.region, this.repartidor.preview.id).on('child_added', snapshot => {
      this.ngZone.run(() => {
        this.commonService.dismissLoading()
        const status = snapshot.val()
        this.repartidorService.cleanResult(this.region, this.repartidor.preview.id)
        this.repartidorService.result(this.region, this.repartidor.preview.id).off('child_added')
        switch (status) {
