...
export class RepartidoresService {
  ...
  guardaRepartidor(region: string, repartidor: RepartidorInfo) {
    return new Promise(async (resolve, reject) => {
      await this.db.object(`nuevo_repartidor_externo/${region}/${repartidor.preview.id}`).set(repartidor)
      resolve()
    });
  }

  result(region: string, idRepartidor: string) {
    return this.db.object(`result/${region}/${idRepartidor}`).query.ref
  }

  cleanResult(region: string, idRepartidor: string) {
    this.db.object(`result/${region}/${idRepartidor}`).remove()
    this.db.object(`nuevo_repartidor_externo/${region}/${idRepartidor}`).remove()
  }
